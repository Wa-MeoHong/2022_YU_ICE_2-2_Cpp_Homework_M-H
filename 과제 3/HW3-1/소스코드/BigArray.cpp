...
{
	num_elements = n;										// 먼저 n으로 설정
	big_array = genBigRandArray(num_elements, OFFSET);		// big_array 생성
	if (big_array == NULL)									// 만약 생성이 되지 않았다면
	{
...
int* BigArray::genBigRandArray(int size, int offset)			// 난수 생성 함수
{
	RANDOMIZE();												// 랜덤 시드 생성
	int* bigRand = NULL;
	
	bigRand = new int[size];				// num_elements(생성할 랜덤한 수 갯수)만큼 동적할당 
...
void BigArray::suffle()								// 섞어주기 (셔플) 함수
{
	...
	while (i != num_elements)										// i가 난수 개수까지
	{
		// 바꿀 숫자를 2개 선정함. 선정은 RAND함수로 결정
...
void BigArray::getStatistics(Statistics& stats)
{
	...
	stats.avg = sum / num_elements;						// stats 구조체에 평균 저장
	
	for (int j = 0; j < num_elements; j++)
		dev2sum = pow((big_array[j] - stats.avg), 2);	// 편차 제곱의 합을 구함
	stats.var = dev2sum / num_elements;					// 분산 = 편차 제곱의 합의 평균
	stats.std_dev = sqrt(stats.var);					// 표준편차 - √분산
}
void BigArray::fprintStatistics(ostream& fout)			// 계산 결과 파일출력
{
	Statistics stats;								// 결과 구조체 생성

	fout.setf(ios::fixed); fout.setf(ios::showpoint); fout.precision(2);
	// fout의 출력 포맷 자리수를 먼저 세팅
	getStatistics(stats);							// 연산결과를 구함
	// 출력
...
void BigArray::fprintBigArray(ostream& fout, int num_per_line)	// 파일 출력 함수
{
	int count = 0;												// 출력할 때 필요함
	int num_lines = 3;											// 중간 생략할 시, 몇줄만 출력하게 하는가에 필요

	/*				>>> 출력 방식 <<<
		1. num_elements(난수의 개수)에 따라 출력방식을 결정한다.
	...
	*/
	...
	for (int i = 0; i < num_lines; i++)
	{
		...
		{
			if (count == num_elements)							// 전부 출력했으면 탈출
				break;
			fout << setw(7) << big_array[count];				// 출력, 자리수를 7로 지정해, 7자리
			count++;
		}
	...
	if (num_elements >= BIG_SIZE)
	{
		...
		for (int i = 0; i < num_lines; i++)
		{
			for (int j = 0; j < num_per_line; j++)
			{
				fout << setw(7) << big_array[count];			// 출력
				count++;
			}
			fout << endl;										// 한줄 띄우기
		}
