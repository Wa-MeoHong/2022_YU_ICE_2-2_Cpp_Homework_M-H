...
public:
	...
	void insert(int i, T_Entry<K>& element);		// 힙에 데이터를 집어넣음
	void remove(int i);								// 데이터 삭제
	void set(int i, T_Entry<K>& element);		// 안에다가 데이터 세팅
	...
	T_Entry<K>& at(int i);						// i번째 있는 데이터 반환 (pointing)
	T_Entry<K> Min(int begin, int end);			// 배열의 Min값 반환
	T_Entry<K> Max(int begin, int end);			// 배열의 Max값 반환

	// member functions
	void shuffle();									// Shuffling 
	int sequential_search(T_Entry<K> search_key);	// 순차 탐색
	int binary_search(T_Entry<K> search_key);		// 이진 탐색
	...
	T_Entry<K>& operator[] (int index) { return t_GA[index]; }			// 배열연산자 []


protected:
	T_Entry<K>* t_GA;		// T_Entry 배열
...
inline void _quick_sort(T_Entry<K>* Array, int Size, int left, int right, SortDirection sd);
// 퀵정렬 본 함수
template<typename K, typename V>
inline void _partition(T_Entry<K>* Array, int Size, int left, int right, int PivotIndex, SortDirection sd);
// 파티션 나누기 함수 

/*				constructor ( 생성자 )
		1. n, nm만 인수를 받는 constructor		*/
template<typename K>
inline TA_Entry<K>::TA_Entry(int n, string nm)
	: capacity(n), name(nm)
{
	num_elements = 0;
	t_GA = new T_Entry<K>[capacity];
}

/*			destructor ( 소멸자 )			*/
template<typename K>
inline TA_Entry<K>::~TA_Entry()
{
	if (t_GA != NULL)
...
/*				getter, setter ( 멤버 함수 중, getter, setter )
		...
		5. at(i)		( i번째 원소를 반환함 )			*/
template<typename K>
inline void TA_Entry<K>::reserve(int new_capacity)		// 새로운 용량의 배열을 재할당 하는 함수
{
	if (this->Size() < this->capacity)
		return;					// 아직 용량이 충분하므로 할 필요가 없다

	T_Entry<K> * oldArray = this->t_GA;			// 새로운 배열을 만들기 위해 오래된 배열을 옮김
	this->t_GA = new T_Entry<K>[new_capacity];		// 새로운 배열 동적할당
...
template<typename K>
inline void TA_Entry<K>::insert(int i, T_Entry<K>& element) // i번째에 원소를 집어넣음
{
...
template<typename K>
inline void TA_Entry<K>::remove(int i)		// i번째의 값을 삭제함
{
	// 먼저 값을 없애준다. 
...
template<typename K>
inline void TA_Entry<K>::set(int i, T_Entry<K>& element) // i번째의 값을 element로 바꿈
{
...
template<typename K>
inline T_Entry<K>& TA_Entry<K>::at(int i)	// 배열의 i번째 값을 반환
{
...
template<typename K>
inline T_Entry<K> TA_Entry<K>::Min(int begin, int end)		// 최소값
{
	// 최소값 초기값 설정
	T_Entry<K>& minValue = this->t_GA[begin];
...
template<typename K>
inline T_Entry<K> TA_Entry<K>::Max(int begin, int end)		// 최대값
{
	// 최대값 초기값 설정
	T_Entry<K>& maxValue = this->t_GA[begin];
...
template<typename K>
inline void TA_Entry<K>::shuffle()
{
...
template<typename K>
inline int TA_Entry<K>::sequential_search(T_Entry<K> search_key)
{
...
template<typename K>
inline int TA_Entry<K>::binary_search(T_Entry<K> search_key)		// 이진탐색
{
...
template<typename K>
inline void TA_Entry<K>::selection_sort(SortDirection sd)
{
	T_Entry<K> compareValue;		// 비교할 값
...
template<typename K>
inline void TA_Entry<K>::quick_sort(SortDirection sd)
{
...
template<typename K>
inline int _partition(T_Entry<K>* Array, int Size, int left, int right, int PivotIndex, SortDirection sd)
{
	int newPI = 0;
	T_Entry<K> PivotValue, temp;
...
template<typename K>		// 파일 출력
inline void TA_Entry<K>::fprint(ostream ostr, int elements_per_line)
{
...
template<typename K>		// 파일 출력 (n줄만 출력후 나머지 생략)
inline void TA_Entry<K>::fprintSample(ostream ostr, int elements_per_line, int num_sample_lines)
{
...
template<typename K>
inline bool TA_Entry<K>::isValidIndex(int i)
{
