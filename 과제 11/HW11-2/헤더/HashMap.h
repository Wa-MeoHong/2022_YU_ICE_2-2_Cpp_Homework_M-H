...
inline typename HashMap<K, V>::Iterator HashMap<K, V>::insert(const K& k, const V& v)
{
	...
	{
		p.ent->SetVal(v);				// 키를 찾았다면 Entry의 키 값을 이걸로 변경한 후, 반환
		// Iterator의 setValue 
		return p;
	}
}
...
inline typename HashMap<K, V>::Iterator HashMap<K, V>::_find(const K& k)
{
	...
	unsigned int i = hash(k) % this->B.size();			// CyclicShiftHashCode를 사용해 hash의 Value를 계산함
	BItor bkt = this->B.begin() + i;			// 버킷의 시작에서부터 i번째에 있는 값

	Iterator p(this->B, bkt, bkt->begin());
	while (!_endOfBkt(p) && (*p).Key() != k)		// 버킷의 키가 끝에  도달하거나, k가 아닐 때까지
		++p.ent;
	return p;									// p의 값 반환
}
template<typename K, typename V>
...
{
	++(this->ent);						// 먼저 ent를 다음 entry로 변경한다.
	if (_endOfBkt(*this))				// 만약 지금 현재 버킷 내부에서 end가 났다면( ent가 end())
	{
		++bkt;							// BItor 이동 ( vector Iterator )
		while ((this->bkt != this->ba->end()) && (this->bkt->empty()))	// 비어있지 않은 bkt을 찾을때까지 이동
			++bkt;
		if (this->bkt == this->ba->end())				// 만약 bkt이 Hash Map의 end에 도달했다면 바로 반환
			return *this;
		this->ent = this->bkt->begin();					// 아니라면, ent의 값을 bkt의 begin()과 일치시킨다.
	}
	return *this;						// 반환
