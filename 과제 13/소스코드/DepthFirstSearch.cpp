...
// consructor 
DepthFirstSearch::DepthFirstSearch(Graph& g) : graph(g), done(false)
{
	...
	// 거리 표 초기화
	ppConnectivity = new double* [num_nodes];
	for (int i = 0; i < num_nodes; i++)
		ppConnectivity[i] = new double[num_nodes];
	for (int i = 0; i < num_nodes; i++)
		for (int j = 0; j < num_nodes; j++)
...
void DepthFirstSearch::showConnectivity(ostream& ostr)	// 출력문
{
	int num_nodes = graph.NumVertices();
	double dist;
	...
	// 제목부분 (열방향 노드이름 출력)
	ostr << "Connectivity of graph : " << endl;
	ostr << "     |";
	for (int i = 0; i < num_nodes; i++)
	...
	for (int i = 0; i < num_nodes; i++)
	{
		ostr << setw(4) << pVrtxArray[i].Name() << " |";
		for (int j = 0; j < num_nodes; j++)
		{
			dist = graph.DistMtrx()[i][j];
			if (dist == PLUS_INF)
				ostr << setw(5) << "+oo";
			else
				ostr << setw(5) << dist;
